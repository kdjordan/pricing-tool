<template>
	<div class="flex flex-col items-center pt-8 h-full">
		...
		<div
			class="flex flex-col justify-between w-2/3 bg-muted p-4 rounded-xl h-[calc(100vh-70%)]"
		>
			<div v-if="!report" class="flex flex-grow space-x-4 mb-8">
				<UploadComponent
...
<script setup lang="ts">
	...
	const isAZfull = computed(() => uploadedFiles.value.size === 2);

	function handleFileUploaded({ componentName, fileName }: { componentName: string; fileName: string }) {
		uploadedFiles.value.set(componentName, fileName);
	}

	function handleUploadError({ componentName, error }: { componentName: string; error: unknown }) {
		console.error(
			`Error uploading file for ${componentName}:`,
			error
		);
	...
	async function resetThisReport() {
		await resetReportApi('az' as DBName);
		dbStore.resetFilesUploadedByDBname('az' as DBName);
		report.value = null;
	}
